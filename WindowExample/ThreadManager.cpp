...
ThreadManager* ThreadManager::tm = new ThreadManager(10);
std::queue<workPackage> ThreadManager::jobs = std::queue<workPackage>();
std::mutex ThreadManager::mx;
        ...
        if(!ThreadManager::jobs.empty()){ // if there are jobs to be done
            ...
            if(f){ // if found
                ...
                *f = new std::future<void>;
                ...
                workPackage wp = jobs.front();
                
                // launch async thread with next job on queue
                **f = std::async(std::launch::async, wp.func, wp);
...
bool ThreadManager::addJob(workPackage wp){
    // if theres jobs on the queue
    // dont bother checking for free threads
    if(!ThreadManager::jobs.empty()){
        ThreadManager::jobs.push(wp); // push new job/workpackage on queue
        return false;
    }
    ...
    if(f == 0){ // if there's no free thread
        ThreadManager::jobs.push(wp); // push new job on queue
        return false;
    }else{
